...
- Before any push: `godmode precheck --preflight --suite-shards 4` validates HEAD in a disposable worktree - the designated suite, the workflow's own gate steps read from `.github/workflows/godmode-verify.yml`, the banned-term scan - and records a `preflight` attestation at that commit. `godmode authorize stage` refuses to stage a push or a release without a green one at HEAD; `--without-preflight "<reason>"` is recorded, never silent.
- Cutting a release: `godmode changelog merge --set-version X`, `godmode release-notes build X` derives the note from that section and `godmode release-notes check X` holds it to the shape (present, every entry covered, no empty section, no process narration, a Verifying section), `godmode version --reconcile`, `godmode bindings --write`, `godmode release` compares two releases, `godmode checksums` and `godmode sbom` record what shipped, `godmode license` classifies what was read.
- After a source edit: `godmode retest` lists every test that pins a changed file (names its path, module, or stem) as one command per runner; `godmode retest --run` executes it and attests the exit code, so the done bar can cite `retest` instead of a hand-picked test.
- Before closing a session on a project that declares debt counters (`.godmode-ratchets.json`, name to command): `godmode ratchet run` records each value and names every counter that rose; `integrity` carries the same finding.
- Held-back checks: `godmode oracle hold --command "<check>"` (operator password) keeps a check under the git metadata directory where the agent never chose it; `godmode oracle list` and `godmode oracle run` read and run them, and `claim --verify` runs them after the cited checks - a red one caps the grade at observed.
...
- Configuration and environment: `godmode config check`, `godmode operator --policy` (which layer decided each gate), `godmode environment`, `godmode locale check`, `godmode extensions`, `godmode paired-artifact` (files that must stay in sync), `godmode parity` and `godmode upstream` against the reference repository (`upstream --path <tree> --skills <keyword>` lists its skill docs that touch a keyword, the parity read before a verdict), `godmode freshness` for research that aged.
