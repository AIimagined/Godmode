...
- A verified claim whose cited check was run and attested by the same agent carries a `self-attested` advisory; the grade stands on the exit code, and an attestation from another agent or model makes it independent.
...
- Required sources count reads seen in the host transcript (`session open --transcript`; the session-start hook passes it), with the path comparison folding case on Windows, so a file read this turn is not reported unread.
